...
def rotr(v, s): return ((v >> s) | (v << (32 - s))) & M
...
def process(block, state):
    ...
    return [(state[i] + v) & M for i, v in enumerate([a, b, c, d, e, f, g, h])]

def sha256(data: bytes) -> str:
    state = H0[:]
    for off in range(0, len(pad(data)), 64):
        state = process(pad(data)[off:off+64], state)
