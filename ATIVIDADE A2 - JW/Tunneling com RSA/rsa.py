...
def cifrar(texto: str, pub: tuple) -> str:
    e, n = pub
    return json.dumps([pow(ord(c), e, n) for c in texto])

def decifrar(cifra: str, priv: tuple) -> str:
    d, n = priv
    return ''.join(chr(pow(b, d, n)) for b in json.loads(cifra))
